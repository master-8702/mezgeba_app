...
def individual_registrar_detail(request, pk):
    ...
    # here in the try block we will check if that specific object (registrar) exists
...
# after receiving the data we will validate it and if evrything is well we will create a new Individual Registrar based on
...
def individual_registrar_delete(request, pk):
    # here we will delete the registrar that already exists in the system (if it exists) and return HTTP status code 200 
